 ...
 * @ModuleBegin 3db
 * 3db = 3P + Data Base
 * 3P = Personal, Portable, Protected |
 *      Plug and Play Passwords
 * 3db is an 'Action DB'. It stores user actions.
 */
/*
 * 3db Storage features:
 ...
 *  3. iOS and Android software will be available (later) that will make the data
 ...
 *     single file - this allows 3db to leverage the filesystems the structure
 ...
 * 4. Distributed. This is related to the Portability feature of 3db.
 *    New data is meant to be generated concurrently on different DBs and can be
 ...
 *    of 3db. The DB can be accessed concurrently by different clients with
 ...
 *
 * Notes on 3db storage design:
 * 1. All 3db data is stored as files in a file-system like hierarchical
 *      structure. 3db is a collection of action-records. Each action-record
 *      represents a user action on the timeline. Each set of action-records
 *      belongs to a Journal. The Journal represents an underlying data-dictionary.
 *      The dictionary is obtained by replaying all the actions of a journal in
 ...
 *      store. Each node of the dictionary - called data-item - has the followng
 *      components:
 ...
 *      Correspondingly, each action-record maps on to a single data-item.
 *      Multiple action-records will map to a given data-record. Each
 *      action-record is a JSON object having the following components:
 ...
 *      Each Journal is expected to contain action-records of the same data type,
 ...
 *      if the application chooses to do so. The 3db semantics is only concerned
 *      with managing data access,  merging and cleanup operations; it doesn't
 *      care about the values carried in each action-record.
 * 2. The root folder has the suffix '.3ab'. The name of the root directory
 *      has the format <DB Name>.3ab. Where <DB Name> is a user-friendly
 *      name given to the DB. This should allow multiple DBs to exist
 *      side-by-side on the same file-system. The DB Name is chosen by
 *      the end-user and if it happens to have characters not allowed by the
 *      file-system, then the DB name is derived by converting the given name
 ...
 *      you could merge two DBs with different names - as long as you had
 *      the password for each.
 * 3. Merge2Sync: 3db/3ab is an actions base. Its atomic fundamental concept is an 
 *      action-record. It is a collection of action-records. Everytime the DB is
 *      'opened' by a client-device, all actions are 'replayed' and reconciled.
 ...
 *      through the values). This makes it possible to mix actions generated by
 *      different devices. This is the cornerstone of the Merge2Sync concept.
 *      Action-records of different DBs can just be thrown together in order to
 *      achieve a merging of the DBs. Later, when a client reconciles the
 ...
 * 4.   Direcotries files in the DB each have a known suffix. Each suffix 
 *      indicates a file-type or directory-type.
 *      1. Directory with suffix .3ad: Inside the root folder are directories, each
 *          holding actions of one Dictionary. Think of this like tables inside
 *          a DB. Each dictionary in the DB has its own directory at the top-level.
 *          The name of the directory is of the form <Dictionary Name>.3ad. At
 *          this time there will be the following dictionaries:
 *          1.  'K' for 'Knowledge' dictionary - K.3ad,
 *          2.  'P' for 'password' dictionary - P.3ad and perhaps
 *          3.  'B' for 'bookmarks' dictionary - B.3ad.
 *      2. File "about.3at": This is a file that holds metadata regarding the DB
 *          such as its name. It is a JSON UTF-8 file. There is one file in the
 *          root folder and one inside each dictionary directory.
 *      3. Files with suffix '.3ao': These files lie inside a dictionary '.3ad'
 *          directory. This suffix indicates an action-file that is open
 *          for appends. This is a journal-file that contains a list of action
 *          records. New records may be appended to such file. Each Journal in
 *          the DB shall have at the most one .3ao file per device, into wich all new
 *          records of the Journal shall be appended. After a .3ao file reaches
 *          a certain size or age, it is'closed' by renaming it like a .3ac file
 *          (see below). Its filename has the following format:
 * 
 *          APPROACH 1:
 *          <Device-ID>.3ao. <Device-ID> is a device-specific identifier which
 *          will not change over time.
 *          generated by the client to avoid file-name clashes in case the date
 *          and timestamp matched. There should normally be only one .3ao file
 *          per dictionary. If one is not found, then the client will create
 *          it. If more than one is found, then they must be from different devices
 *          in cases where this is possible - such as if the files were inside
 *          a synchronized directory like Google Cloud Drive. In such a case the
 *          client will treat that as a separate DB and Merge those entries into
 *          its own file (identified by <Device-ID>). This would be an import
 *          rather than a two-way merge (see Merge2Sync below) because the client
 *          as a rule would not touch the other .3ao files (otherwise there could
 *          be conflicts in the Sky drive synchronization). So in case two devices
 *          were sharing the DB in a Sky Drive, they would both keep importing
 *          the other's new entries into their own files. This would cause automatic
 *          synchronization between the two sides ! Afer one year of 'inactivity'
 *          the files can be deleted by any device (for the sake of cleanup).
 * 
 *          APPROACH 2:
 *          open.3ao. Only one file per dictionary - with fixed name. If more than one is found,
 *          it will naturally have a different name - then their contents get consolidated into
 *          open.3ao and remaining files deleted. Or, they get converted to 3ac files if they're
 *          big enough. This has
 *          the advantage that if the user carries the DB in a memory stick and
 *          uses it on different PCs, they will all continue to use the single
 *          file for writes and we won't have any data duplication as compared
 *          to the approach 1.
 *          However, the following will happen with Sky Drives.
 *          Device A works on the file open.3ao. Concurrently device B works on the
 *          file. Then the Google drive synchronizes. A conflict will be detected.
 *          Sky Drive will not auto-merge. Instead, it will rename one of the two files
 *          to -say- open(2).3ao. These will be sync'd to both sides each of which will
 *          consolidate the two files into open.3ao on both sides. Both sides
 *          will now generate identical files and hence the skydrive will hopefully
 *          not trigger a conflict the next time. Will need to check specifics of
 *          each skydrive.
 *          Additionally, files get promoted to 3ac satus after
 *          they grow to a certain size or age (aging them out will cause too
 *          many small files, but will make DB merges faster though it can be
 *          argued that merges take place once in a while and therefore people
 *          are not that concerned with a few extra seconds to merge). In any case,
 *          approach 2 is simpler and therefore better.
 *          
 *      4. File suffix '.3ac': This suffix indicates a 'closed' journal-file.
 *          This is a journal-file that contains a list of action-records
 *          New records may not be appended to such file. However, while
 *          'cleaning up' the EB, dead action records may be removed from
 *          such files. These action records would be ones that represent data
 *          that was later removed or changed - as evidenced by a delete
 *          action record further down the time-line in either the same file or a
 *          different file. A .3ac file name has the following format:
 *          Journal-Name.<Date&Time>.<UID>.3ac. <Date&Time> is the <Date&Time>
 *          of creation (not modification) - this will allow easy sorting of
 *          actions in chronological order. It will also make it very unlikely
 ...
 *          to allow multiple 3ac files to exist in an dictionary - including the case
 *          where the files were generated by different devices.
 *      5. More about action (.3ao and .3ac) files:
 *          These are JSON files containing a list of action-records in JSON
 *          syntax. The contents of the file are a JSON array of JSON objects. 
 *          Each object represents an action-record (dt: P-Record or 
 *          dt: K-Record). In the case of 3ao files, the closing ']' will be
 *          absent and will need to be appended before the file is fed to
 *          JSON.parse. On the other hand content of 3ac files can be directly
 *          fed to JSON.parse. TBD: Encryption.
 *      6. File Suffix '.3am': This is a JSON file generated by converting
 *          the in-memory Data-Dictionary into JSON text. This file will be
 *          loaded by the client instead of replaying the action-files, if no
 *          3ac files were added after generation of this file. The filename
 *          follows the format <Journal Name>.3am.
 * 5. All the files pertaining to a given dictionary are kept inside a subfolder
 *          of the root called <Dictionary Name>.3ad. This is the dictionary's
 *          root folder. This folder contains all the .3ao, .3am and .3ac files
 *          pertaining to its dictionary/journal. At the time of this writing
 *          there are expected to be the folders K.3ad and P.3ad.
 ...
 *          selecting
 *          the option to not overwrite any 3ac files. The result of this is
 *          that the target folder will get all 3ac files and, no files
 *          will be clobbered. Since each 3ac file has a <UUID> in it, having a
 ...
 *          if the filenames are same, then that data is the same because 3ac
 *          files are unchanging. If however, there was a clash of 3ao
 *          filenames, then the largest one must be kept since that certainly
 *          has newer data (3ao files are never pruned). However, the two EBs
 ...
 *          2. Close all 3ao files in EB1 and EB2.
 *              2.1 Alternatively, concatenate all .3ao files in EB1 and EB2
 *              combined into a single file (with new name). Remove all the 
 *              old .3ao files. Optionally compact the concatenated .3ao file.
 *              Copy the resulting 3ao file to both sides.
 *          3. Copy all 3ac files of EB1 not known to EB2 into EB2.
 *          4. Copy all 3ac files of EB2 not known to EB1 into EB1.
 ...
 *          since previously merged 3ac files won't be copied this will ensure
 ...
 *          as noted above, 3ao files are converted to 3ac files after a certain
